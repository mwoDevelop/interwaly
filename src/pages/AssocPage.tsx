            ...
            <div key={interval.id} className="rounded-md border border-slate-800 bg-slate-900 p-4 text-sm text-slate-200">
              ...
              <ul className="mt-3 space-y-1 text-xs text-slate-400">
                {interval.songs.map((song) => (
                  <li key={song.title}>
                    <p className="font-medium text-slate-300">{song.title}</p>
                    <p className="text-slate-500">
                      {song.direction === 'up' && <span className="text-emerald-400">▲ w górę</span>}
                      {song.direction === 'down' && <span className="text-rose-400">▼ w dół</span>}
                      {song.direction === 'static' && <span className="text-sky-400">◼ stały dźwięk</span>}
                      <span className="ml-2 text-slate-400">{song.fragment}</span>
                    </p>
                  </li>
                ))}
              </ul>
